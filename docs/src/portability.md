...
|-------|------|----------|
| Debug | ~50 KB | Any |
| Release (x86_64) | ~9 KB | Windows/Linux/macOS |
| Release (ARM MCU) | ~4 KB | Cortex-M4 |

The tiny size comes from:
1. `no_std` — no libstd bloat
2. `panic = "abort"` — no unwind tables
3. `strip = "symbols"` — no debug info
4. `lto` + `codegen-units=1` — dead code elimination
